...
plot(density(Chlorella_mod$TCS), main = "Figura 14. Función de densidad para la variable Tasa de Crecimiento Simple", y="Densidad")
plot(density(Chlorella_mod$Temperatura), main = "Figura 15. Función de densidad para la variable Temperatura (°C)", y="Densidad")
plot(density(Chlorella_mod$CO2), main = "Figura 16. Función de densidad para la variable CO2 (mg/ml))", y="Densidad")
plot(density(Chlorella_mod$OD), main = "Figura 17. Función de densidad para la variable Oxígeno disuelto (mg/ml)", y="Densidad")

plot(ecdf(Chlorella_mod$Densidad), main = "Figura 18. Función de probabilidad acumulada para la variable Densidad (cel/ml)")
plot(ecdf(Chlorella_mod$TCS), main = "Figura 19. Función de probabilidad acumulada para la variable Tasa de Crecimiento Simple")
plot(ecdf(Chlorella_mod$Temperatura), main = "Figura 20. Función de probabilidad acumulada para la variable Temperatura")
plot(ecdf(Chlorella_mod$CO2), main = "Figura 21. Función de probabilidad acumulada para la variable CO2")
plot(ecdf(Chlorella_mod$OD), main = "Figura 22. Función de probabilidad acumulada para la variable Oxígeno Disuelto")
...
```{r}
pairs.panels(Chlorella_mod[,], smooth = TRUE, density = TRUE, method="pearson", font=2, main="Figura 23. Matriz de correlaciones")


Tabla_Densidad <- Chlorella_mod %>% filter(FBR == "Helicoidal") %>%
  summarize("Media" = mean(Densidad),
            "SD" = sd(Densidad),
            "Máximo" = max(Densidad),
            "Mínimo" = min(Densidad))

Densidad%>% 
  mutate(Promedio_Densidad = mean(Densidad))
```
